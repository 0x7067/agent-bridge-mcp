...
- **WHEN** a caller invokes `task_preview` with an unsupported argument such as `maxTurns` inside `arguments`
...
- **WHEN** a caller invokes `task_preview` with `_meta` on the envelope and an unsupported field inside `arguments`
