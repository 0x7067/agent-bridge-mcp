...
- **WHEN** a caller uses task lifecycle tools such as `agent_status`, `agent_logs`, `agent_wait`, or `agent_result`
...
- **WHEN** a caller invokes `agent_preview` for a reduced launch profile
