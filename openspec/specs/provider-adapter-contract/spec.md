...
- **WHEN** a caller invokes `doctor` with `focus: "providers"` and without smoke probes
...
- **WHEN** a caller invokes `doctor` with `focus: "providers"` and smoke probes enabled
...
- **WHEN** a caller uses task lifecycle tools such as `agent_observe` or `agent_result`
...
- **WHEN** a caller invokes `agent_spawn` with `dryRun: true` for a reduced launch profile
