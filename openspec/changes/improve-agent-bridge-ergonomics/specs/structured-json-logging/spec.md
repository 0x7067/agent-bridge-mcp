...
## Purpose
Instrument the task actor, provider launcher, and IO drainer boundaries with `tracing` spans and events emitted as newline-delimited JSON to stderr, enabling correlated telemetry without polluting the MCP stdout transport.
