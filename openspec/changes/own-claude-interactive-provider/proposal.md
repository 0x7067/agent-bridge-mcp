...
- Compatibility: public provider name and lifecycle APIs remain stable, but operator setup changes because production Claude execution should be run through the owned host-runner path. Existing `CLAUDE_P_BIN` configuration becomes legacy/ignored, and `CLAUDE_BIN` is repurposed or replaced for the official interactive `claude` binary as documented by the implementation.
