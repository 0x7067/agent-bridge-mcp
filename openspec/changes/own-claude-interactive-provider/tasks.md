...
- [ ] 2.1 Add an owned Claude runner module that starts official interactive `claude` in a PTY with login-shell-compatible environment setup.
...
- [ ] 2.4 Generate temporary runner-owned `--settings` JSON for `SessionStart` and `Stop` hooks without editing durable Claude config.
- [ ] 2.5 Implement runner-owned hook relay IPC with owner-only permissions, bounded reads, cleanup, and no hook stdout leakage into Claude context.
...
- [ ] 3.1 Define and implement host-runner protocol v2 for owned interactive Claude requests/results, including protocol mismatch handling.
...
- [ ] 3.5 Replace print-mode stdout JSON success parsing and smoke-token detection with owned-runner Stop/transcript result parsing.
...
- [ ] 4.5 Re-submit the revised plan for Agent Bridge review before implementation tasks beyond research.
