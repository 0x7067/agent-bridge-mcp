...
The system SHALL define a `FailureCategory` enum encompassing every failure category presently expressed as a string, including `ProviderTimeout`, `ProviderExitError`, `ProviderStartError`, `ProviderOutputError`, `ProviderSandboxDenied`, `HostRunnerUnavailable`, `HostRunnerProtocolMismatch`, `WorktreeCleanupFailed`, `WorktreeReclaimFailed`, `TranscriptUnavailable`, `AgentDirCleanupFailed`, and `ClientDisconnected`.
...
### Requirement: Serialization maps to stable kebab-case strings
The system SHALL serialize `FailureCategory` to lowercase-kebab-case strings matching the historical wire format (e.g., `provider-timeout`). Deserialization SHALL round-trip the same mapping.
...
- **WHEN** the host runner transmits `"provider-timeout"` in a v2 response
