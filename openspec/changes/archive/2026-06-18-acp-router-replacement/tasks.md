...
- [x] 3.3 Promote ACP `stopReason` into task diagnostics so refusal and cancellation classify without transcript text parsing.
- [x] 3.4 Return compact attempt evidence references without embedding raw stdout, stderr, transcript, or diff bodies.
...
- [x] 4.5 Add stdio tests proving MCP default behavior is unchanged and ACP router stdout stays valid JSON-RPC.
...
- [x] 5.2 Add fake-provider tests for routed success, infrastructure failover, refusal, cancellation, auth/billing blockers, and retained evidence.
- [x] 5.3 Update docs and guidance to describe ACP router replacement and MCP lifecycle migration compatibility.
- [x] 5.4 Run `cargo test -p agent-bridge-mcp -- --test-threads=1`.
- [x] 5.5 Run `scripts/quality.sh`.
