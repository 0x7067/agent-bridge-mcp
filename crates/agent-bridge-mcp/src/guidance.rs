...
## ACP router migration

`agent-bridge-mcp` with no subcommand is the replacement prompt-turn contract. It is a separate ACP runtime path, not a ninth MCP tool.

The router handles ACP `initialize`, `session/new`, and `session/prompt`. A prompt turn asks the router once; v1 policy candidates are `codex` and `claude`. The response carries a compact `routerResult` with `terminalKind` (`answer`, `blocker`, or `failure`), selected provider, attempts, diagnostics, `failoverTrail`, and evidence refs. Normal router results do not embed raw stdout, stderr, transcript, or diff bodies.

The MCP lifecycle remains migration compatibility and internal machinery while ACP-router clients adopt the replacement surface. It still works for caller-managed delegation, but it is not the final product contract for routed native collaboration.
"#;
