...
fn tool_call_params_parse_known_tool_names() {
    let params: ToolCallParams = serde_json::from_str(
        r#"{"name":"task_preview","arguments":{"provider":"codex","mode":"review","prompt":"x"}}"#,
    )
    .unwrap();
    assert_eq!(params.name, ToolName::TaskPreview);
}

#[test]
fn task_preview_input_rejects_unknown_fields() {
