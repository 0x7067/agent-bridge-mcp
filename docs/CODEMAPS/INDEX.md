...
block-beta
    columns 3
    MCP["MCP Client<br/>(stdio)"]:1
    RT["Runtime<br/>runtime.rs"]:1
    SR["Server Router<br/>server.rs"]:1
    space:2
...
| 2 | `src/server.rs` + diagnostics.rs | Very High | Yes |
| 3 | `src/provider.rs` | High | Yes |
| 4 | `src/claude_interactive/` | Medium-High | Yes |
| 5 | `src/guidance.rs` | Medium | Yes |
| 6 | `src/runtime.rs` | Low | Yes |
| 7 | `src/mcp.rs` | Low | Yes |
| 8 | `src/domain.rs` | Low | Yes |
| 9 | `src/claude_host.rs` | Medium | Yes |
| 10 | `src/tools.rs` | Medium | Yes |

## Codemaps

| Area | File | Description |
|------|------|-------------|
| Backend | [backend.md](backend.md) | Server router, task lifecycle, runtime, and domain types |
