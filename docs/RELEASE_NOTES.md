...
- `agent_spawn` accepts optional `retryPolicy` with `maxRetries` and `backoffMs`. Retries apply only to transient categories: `provider_timeout`, `provider_start_error`, and `host_runner_unavailable`.
